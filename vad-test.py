...
CHUNK_SIZE = int(framerate*CHUNK_DURATION_MS / 1000)
PADDING_DURATION_MS = 1500   # 1 sec jugement

CHUNK_BYTES = CHUNK_SIZE * 2  # 16bit = 2 bytes, PCM
NUM_PADDING_CHUNKS = int(PADDING_DURATION_MS / CHUNK_DURATION_MS)
# NUM_WINDOW_CHUNKS = int(240 / CHUNK_DURATION_MS)
NUM_WINDOW_CHUNKS = int(480 / CHUNK_DURATION_MS)  # 400 ms/ 30ms  ge
...
e.pack(ipadx=180,ipady=3)

label = Label(root)
label['text'] = '单次检测超时时间 (秒)'
label.pack()
...
def playandrecord(mesrure_index):
    ...
    while not got_a_sentence:
        ...
        if not triggered:
            ...
            if num_voiced > 0.9*NUM_WINDOW_CHUNKS:
                ...
                time_1 = (endtime - starttime).seconds * 1000000 + (endtime - starttime).microseconds - 350*1000 #检测所需时常为350ms，如果检测到音频开始端点，应将360ms剔除
                t.insert('1.0', "%4d : %f ms\n" % (mesrure_index+1, float(time_1 / 1000)))
    ...
    if is_save_file:
        ...
        t.insert('1.0', "%4d : %s saved\n"%(mesrure_index+1,filename))
    streami.close()
    pai.terminate()
    #start to PESQ
    #if is_save_file:
